const { Chart } = require("chart.js"); // Deixar no arquivo view

// Variaveis para codigo
let nPontosGrafico = 72;
let delayDados = 10;
...
window.onload = function () {
  db.ref(uid)
    ...
    .on("value", (snapshot1) => {
      ...
      document.getElementById("ultimoDado").innerHTML =
        "Ultima Atualização: " +
        data.getDate() + // Imprime a data do ultimo timeStamp
        "/" +
        (data.getMonth() + 1) +
        "/" +
        data.getFullYear() +
      ...
      db.ref(pathBase + "UmidL401").on("value", (snapshot) => {
        document.getElementById("umidade1").innerHTML = snapshot.val() + "%"; // Muda o valor da umidade1 (LAB401) no HTML
      ...
      db.ref(pathBase + "UmidL402").on("value", (snapshot) => {
        document.getElementById("umidade2").innerHTML = snapshot.val() + "%"; // Muda o valor da umidade2 (LAB402) no HTML
      ...
      db.ref(pathBase + "UmidL403").on("value", (snapshot) => {
        document.getElementById("umidade3").innerHTML = snapshot.val() + "%"; // Muda o valor da umidade3 (LAB403) no HTML
      ...
      db.ref(pathBase + "UmidL404").on("value", (snapshot) => {
        document.getElementById("umidade4").innerHTML = snapshot.val() + "%"; // Muda o valor da umidade4 (LAB404) no HTML
      ...
      db.ref(pathBase + "UmidL405").on("value", (snapshot) => {
        document.getElementById("umidade5").innerHTML = snapshot.val() + "%"; // Muda o valor da umidade5 (LAB405) no HTML
  ...
  let tipoPessoa = document.querySelectorAll('input[name="numPontos"]');
